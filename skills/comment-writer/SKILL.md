---
name: comment-writer
description: "Write warm, direct collaboration comments. Trigger: PR feedback, issue replies, reviews, or GitHub/GitLab comments."
license: Apache-2.0
metadata:
...
- GitHub/GitLab PR or issue comments.
...
| Match target context language | Write in the target context language by default: Spanish issue/thread → Spanish comment, English issue/thread → English comment. If the user explicitly requests a language or tone, follow that request. For Spanish comments, use neutral/professional Spanish by default unless the target context clearly calls for regional tone. |
...
```markdown
This PR exceeds the 400-line budget, so we need to split it or justify why not.
